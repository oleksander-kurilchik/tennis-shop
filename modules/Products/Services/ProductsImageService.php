...
namespace TrekSt\Modules\Products\Services;
...
{
    protected static $diskName = 'products_images';
    protected static $imageNamePrefix = 'products_images';
    ...
    protected static $pathMini = 'mini';

    protected static $bigSize = ['width' => 1024, 'height' => 1024];
    protected static $mediumSize = ['width' => 635, 'height' => 635];
    protected static $smallSize = ['width' => 320, 'height' => 320];
    protected static $miniSize = ['width' => 150, 'height' => 150];
    ...
    public function deleteImage($name)
    {
        ...
        if (self::disk()->exists(self::$pathMini.'/'.$name)) {
            self::disk()->delete(self::$pathMini.'/'.$name);
        }
        ...
        if (self::disk()->exists(self::$pathMini.'/'.$name.'.webp')) {
            self::disk()->delete(self::$pathMini.'/'.$name.'.webp');
        }
...
//                $constraint->upsize();
        ...
        $imageMini = \Image::make($originalImage)->fit(self::$miniSize['width'], self::$miniSize['height'],
            function ($constraint) {
                $constraint->aspectRatio();
//                $constraint->upsize();
            });
        self::disk()->put(self::$pathMini.'/'.$name, $imageMini->encode());
        self::disk()->put(self::$pathMini.'/'.$name.'.webp', $imageMini->encode('webp'));
...
//
    ...
    public static function getImagePath0X($name)
    {
        return self::disk()->url(self::$pathMini."/".$name);
    }
