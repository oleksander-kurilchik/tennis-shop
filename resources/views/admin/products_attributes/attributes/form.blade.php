<div class="form-products-wrapper">
    ...
    @foreach($languages as $language)
        <div class="form-group {{ $errors->has('title.'.$language->value) ? 'has-error' : ''}}">
            ...
            <div class="">
                {!! Form::text("title[{$language->value}]", old("name[{$language->value}]",(isset($attribute)?$attribute->getTranslation('title',$language->value,false):'')), ['class' => 'form-control', 'required' => 'required' , 'autocomplete'=>'off']) !!}
    ...
    <div class="form-group {{ $errors->has('order') ? 'has-error' : ''}}">
        ...
        <div class="">
            {!! Form::number('order', old('order',isset($attribute)?$attribute->order:0), ['class' => 'form-control', 'autocomplete'=>'off', 'required' => 'required']) !!}
